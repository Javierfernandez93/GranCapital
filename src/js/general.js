    ...
    showLoader(element,__class)  {
        ...
        if(this.getElement().length > 0)
        {
            ...
            if(this.getElement().get(0).tagName == "BUTTON")
            {
                ...
                this.getElement().html('<div class="progress-mds"><div class="indeterminate"></div></div>');
            }
        }
    }
    removeStyle()  {
...
{
    $(alert_element).removeClass("d-none alert-danger alert-success").addClass(mode).html(html);
}
...
function disccuss(title,subTitle,positive_callback,negative_callback){
    ...
    subTitle = (subTitle) ? subTitle : translate('¿Estás seguro de realizar esta acción?');
    alert = alertCtrl.create({
      title: title,
      subTitle: subTitle,
      buttons: [
...
function verifyall(element,tipo,minlength=false,message=false){
    ...
    if(element.val()==''){
        ...
        if(tipo=="mail"){
            if(!isValidMail(element.val()))
                bandera=false;  
        }else if(tipo=='number'){            
            if(minlength){
                if(isNaN(element.val()) || (element.val().length > minlength) || (element.val().length < minlength))
                    bandera=false;
    ...
    if(bandera==false){
        if($(element).hasClass('inputError') == false)
            alertmesage(message);
        // element.focus();
...
function copyToClipboard(elemento) {
  ...
  $temp.remove();
  alertmesage("Se copio el enlace en el portapapeles ",null,"Comparta el enlace con sus prospectos");
}
...
window.togglePassword = function(element)
{
    ...
    {
        $(element).html('<i class="fas fa-eye-slash"></i>').parent().parent().find("input").attr("type","text");
    } else {
        $(element).html('<i class="far fa-eye"></i>').parent().parent().find("input").attr("type","password");
    }
}
